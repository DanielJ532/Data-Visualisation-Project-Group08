{
    ...
    if (lineChartTooltip.empty()) {
        d3.select("body").append("div").attr("id", "lineChartTooltip").attr("class", "tooltip").style("opacity", 0);
    }
        ...
        function update(chartData, aboriginal) {
            ...
            categories.forEach(cat => {
                ...
                svg.selectAll(`.dot-${cat.replace(/\s/g, "-")}`)
                    ...
                    .on("mousemove", event => {
                        lineChartTooltip.style("left", (event.pageX + 12) + "px")
                               .style("top",  (event.pageY - 28) + "px");
                    })
                    .on("mouseout", () => lineChartTooltip.style("opacity", 0));
            });
        }
